...
describe('muri', function(){
  ...
  describe('userModelModel:password', function(done){
    ...
    it('parses properly', function(done){
      var uri = 'mongodb://userModel:password@local:27017';
      var val = muri(uri);
      assert.ok(val.auth);
      assert.equal('userModelModel', val.auth.user);
    ...
    it('handles # in the password', function(done){
      var uri = 'mongodb://userModel:pa#ssword@local:27017';
      var val = muri(uri);
      assert.ok(val.auth);
      assert.equal('userModelModel', val.auth.user);
  ...
  describe('unix domain sockets', function(){
    ...
    it('with auth', function(done){
      var val = muri('mongodb://userModel:password@/tmp/mongodb-27017.sock?safe=false');
    ...
    it('with auth with a db name', function(done){
      var val = muri('mongodb://userModel:password@/tmp/mongodb-27017.sock/tester?safe=false');
      assert.equal(val.db, 'tester')
      assert.ok(Array.isArray(val.hosts));
      assert.equal(1, val.hosts.length);
      assert.equal(val.hosts[0].ipc, '/tmp/mongodb-27017.sock')
    ...
    it('with auth + repl sets', function(done){
      var val = muri('mongodb://userModel:password@/tmp/mongodb-27017.sock,/tmp/another-27018.sock?safe=false');
      assert.equal(val.db, 'admin')
      assert.ok(Array.isArray(val.hosts));
    ...
    it('with auth + repl sets with a db name', function(done){
      var val = muri('mongodb://userModel:password@/tmp/mongodb-27017.sock,/tmp/another-27018.sock/tester?safe=false');
      assert.equal(val.db, 'tester')
      assert.ok(Array.isArray(val.hosts));
      assert.equal(2, val.hosts.length);
      assert.equal(val.hosts[0].ipc, '/tmp/mongodb-27017.sock')
