...
function Db(databaseName, serverConfig, options) {
  ...
  // State to check against if the userModel force closed db
...
Db.prototype.open = function(callback) {
  ...
  // Check that the userModel has not called this twice
  if(this.openCalled) {
    // Close db
    this.close();
    // Throw error
...
/**
 * Logout userModel from server, fire off on all connections and remove all auth info
 *
...
/**
 * Authenticate a userModel against the server.
 * authMechanism
 * Options
...
/**
 * Add a userModel to the database.
 *
 * Options
 *  - **w**, {Number/String, > -1 || 'majority' || tag name} the write concern for the operation where < 1 is no acknowlegement of write and w >= 1, w = 'majority' or tag acknowledges the write
 *  - **wtimeout**, {Number, 0} set the timeout for waiting for write concern to finish (combines with w option)
...
Db.prototype.addUser = function(username, password, options, callback) {
  ...
  // Fetch a userModel collection
  var collection = this.collection(DbCommand.SYSTEM_USER_COLLECTION);
  // Check if we are inserting the first userModel
  collection.count({}, function(err, count) {
    // We got an error (f.ex not authorized)
    if(err != null) return callback(err, null);
    // Check if the userModel exists and update i
    collection.find({user: username}, {dbName: options['dbName']}).toArray(function(err, documents) {
      ...
      // We have a userModel, let's update the password or upsert if not
...
/**
 * Remove a userModel from a database
 *
 * Options
*  - **w**, {Number/String, > -1 || 'majority' || tag name} the write concern for the operation where < 1 is no acknowlegement of write and w >= 1, w = 'majority' or tag acknowledges the write
 *  - **wtimeout**, {Number, 0} set the timeout for waiting for write concern to finish (combines with w option)
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  // Check if the userModel force closed the command
 ...
 *
 * Options
 *  - **uri_decode_auth** {Boolean, default:false} uri decode the userModel name and password for authentication
