 ...
 *       ]
 *
 *       User.populate(userModel, opts, function (err, userModel) {
 *         console.log(userModel);
 *       })
 *     })
 *
 *     // populates an array of objects
 *     User.find(match, function (err, user) {
 *       var opts = [{ path: 'company', match: { x: 1 }, select: 'name' }]
 *
 *       User.populate(user, opts, function (err, userModel) {
 *         console.log(userModel);
 *       })
 *     })
 *
 ...
 *
 *     var userModel = { name: 'Indiana Jones', weapon: 389 }
 *     Weapon.populate(userModel, { path: 'weapon', model: 'Weapon' }, function (err, userModel) {
 *       console.log(userModel.weapon.name) // whip
 *     })
 *
 *     // populate many plain objects
 *     var user = [{ name: 'Indiana Jones', weapon: 389 }]
 *     user.push({ name: 'Batman', weapon: 8921 })
 *     Weapon.populate(user, { path: 'weapon' }, function (err, user) {
 *       user.forEach(function (userModel) {
 *         console.log('%s uses a %s', user.name, userModel.weapon.name)
...
function populateDocs (docs, options, cb) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if (isDocument && !doc.isModified(path)) {
      ...
      // populated again. because user can specify matcher
      ...
      // signifies the user intent of the state of the path.
 ...
 *      b) remove _id from sub docs when userModel specified
 *
 *    findOne
 *      a) if no doc found, set to null
 *      b) remove _id from sub docs when userModel specified
 *
 * 2) Remove _ids when specified by user query.
 *
 * background:
 * _ids are left in the query even when userModel excludes them so
...
/*!
 * Remove _id from `subdoc` if userModel specified "lean" query option
 */
 ...
 * query sort order if specified by userModel.
 *
 ...
 *   for find, if userModel specified sort order, results are assigned directly
...
function assignRawDocsToIdStructure (rawIds, resultDocs, resultOrder, options, recursed) {
  // honor userModel specified sort order
  var newOrder = [];
